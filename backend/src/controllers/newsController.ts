...
import { fetchLaborNews } from '../services/newsScheduler'; // Import the service logic
...
export const triggerNewsFetch = async (req: Request, res: Response) => {
    try {
        ...
        // Run async (don't wait for full process to respond) or await?
        // Better to await so admin knows it worked.
...
export const triggerNewsFetch = async (req: Request, res: Response) => {
    try {
        await fetchLaborNews();
        res.json({ message: 'Trigger ejecutado con éxito.' });
    } catch (error) {
        res.status(500).json({ error: 'Error al ejecutar trigger.' });
    }
};
